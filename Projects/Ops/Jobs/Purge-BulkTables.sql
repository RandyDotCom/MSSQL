...
/****** Object:  Job [Purge-BulkTables-WTTRR_EDS04]    Script Date: 1/25/2016 1:01:31 PM ******/
IF  EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'Purge-BulkTables-WTTRR_EDS04')
EXEC msdb.dbo.sp_delete_job @job_name=N'Purge-BulkTables-WTTRR_EDS04', @delete_unused_schedule=1
GO
...
SELECT @ReturnCode = 0
/****** Object:  JobCategory [YDPages]    Script Date: 1/25/2016 1:01:31 PM ******/
...
select @jobId = job_id from msdb.dbo.sysjobs where (name = N'Purge-BulkTables-WTTRR_EDS04')
if (@jobId is NULL)
BEGIN
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Purge-BulkTables-WTTRR_EDS04', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'This script will drop table created by cross-datastore scheduling i.e.  tables starting with blk.', 
		@category_name=N'YDPages', 
		@owner_login_name=N'sa', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback

END
/****** Object:  Step [Purge Bulk Schema Tables older than 10 minutes]    Script Date: 1/25/2016 1:01:31 PM ******/
