...
--/****** Object:  Job [Ops TaskManager]    Script Date: 4/11/2018 9:39:58 AM ******/
--IF  EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'Ops TaskManager')
--EXEC msdb.dbo.sp_delete_job @job_name=N'Ops TaskManager', @delete_unused_schedule=1
--GO
...
--select @jobId = job_id from msdb.dbo.sysjobs where (name = N'Ops TaskManager')
--if (@jobId is NULL)
--BEGIN
--EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Ops TaskManager', 
--		@enabled=1, 
...
--EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Schedule from Opshealth', 
--		@step_id=1, 
...
--	select @schedule_id = [schedule_id] from msdb.dbo.sysschedules where name=''Ops Task Manager Rush'' 
...
--		@flags=8
--IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
--/****** Object:  Step [Run Ops TaskManager]    Script Date: 4/11/2018 9:39:58 AM ******/
--IF NOT EXISTS (SELECT * FROM msdb.dbo.sysjobsteps WHERE job_id = @jobId and step_id = 2)
--EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Run Ops TaskManager', 
--		@step_id=2, 
--		@cmdexec_success_code=0, 
--		@on_success_action=1, 
--		@on_success_step_id=0, 
...
--IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
--EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Ops Task Manager Rush', 
--		@enabled=0, 
--		@freq_type=4, 
--		@freq_interval=1, 
--		@freq_subday_type=2, 
...
--IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
--EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Ops Task Manager', 
--		@enabled=1, 
--		@freq_type=4, 
--		@freq_interval=1, 
--		@freq_subday_type=4, 
