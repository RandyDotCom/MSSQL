...
IF  EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'dbaWTTKillBlockingSpids')
EXEC msdb.dbo.sp_delete_job @job_name=N'dbaWTTKillBlockingSpids', @delete_unused_schedule=1
GO
...
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]    Script Date: 2/20/2015 3:50:02 PM ******/
...
select @jobId = job_id from msdb.dbo.sysjobs where (name = N'dbaWTTKillBlockingSpids')
if (@jobId is NULL)
BEGIN
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'dbaWTTKillBlockingSpids', 
		@enabled=1, 
...
END
/****** Object:  Step [Call KillBlockingSpids]    Script Date: 2/20/2015 3:50:02 PM ******/
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Call KillBlockingSpids', 
		...
		@command=N'exec ops.dbo.KillBlockingSpids', 
		@database_name=N'OPS', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
