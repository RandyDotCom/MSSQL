...
BEGIN
	SET NOCOUNT ON;
	/*
	  TO remove a database from this job 
...
select @ERM = 'Backup Type ' + isnull(@BackupType,'NULL') + ' Is not supported (Full or Log)'
select @BackupType = CASE WHEN isnull(@BackupType,'Full') in ('Full','Log','Diff') then isnull(@BackupType,'Full') else 'Error' end  
if @BackupType = 'Error'
Begin 
	Raiserror(@ERM,11,1) 
	Return 0;
...
Begin 
  Begin Try 
	exec msdb.dbo.sp_start_job @job_name='dbaBackupsDailys' 
  end Try 
  Begin catch 
	select @ERM = 'Backup Job may allready be running'
...
Begin 
	Raiserror('Nothing to do, to validate check results execute with @debug=2 ',0,1) with nowait  
end
...
BEGIN
	...
	BEGIN
		...
		if exists(select * from sys.sysprocesses p where cmd='BACKUP DATABASE' and db_name(p.[dbid]) = @dbname)
		BEGIN
		...
		Begin Catch 
			Select @ERM = ERROR_MESSAGE(), @ERN = ERROR_NUMBER(), @ERS = ERROR_SEVERITY() 
			 SELECT @ERM = 'Error Number:' + Cast(@ERN as varchar(20)) + ' Severity:' + Cast(@ERS as varchar(20)) + char(10) + @ERM  
		...
		select @ERM = isnull(@BackupLocation,'@BackupLocation is null') + char(10)
		+ isnull(@filename,'@Filename is null') + char(10) 
		+ isnull(@command,'@Command is null') 
	...
	if @debug is not null 
		...
		select @ERM = null 
		select @ERM = coalesce(@ERM+char(10),'') + isnull([Lineout],'') from @cmdout where lineout like '%The system cannot find the drive specified%'
		if @ERM is not null
		BEGIN
			exec ops.dbo.RaiseAlert @Message=@ERM, @Type='Error',@ErrorID=501 
			exec ops.dbo.sp_help_executesproc 'raiseAlert'
			Raiserror(@ERM,16,1) with nowait 
			Return 0;
		END
